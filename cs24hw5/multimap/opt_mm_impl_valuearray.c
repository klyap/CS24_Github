...
void mm_add_value(multimap *mm, int key, int value) {
    ...
    */
    ...
    //printf("--At key %d, value index %d\n", node->key, node->values_i);
    // My stuff
    if (node->values_i > 0){
        if (node->values_i % 50 == 40){
            ...
            //printf("REALLOC: %d\n", (node->values_i/50 + 1) * 50);
        }
        
        *(node->values + node->values_i) = value;
        node->values_i += 1;
        
        //printf("---Added %d, value index %d\n", *(node->values + node->values_i) + node->values_i, node->values_i);
    } else {
        ...
        *new_value = value;
        ...
        //printf("---Added %d, value index %d == 0\n", *(node->values + node->values_i) + node->values_i, node->values_i);
    }
